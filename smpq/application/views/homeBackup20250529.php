...
<section class="hero  relative w-full bg-black">
    <div class="absolute w-full flex justify-center items-center  top-2 left-0 h-fit z-80">
    <div class="slider-indicator-container w-1/2 flex   bg-gray-200/10 rounded-full">
        <div
    ...
    <div class="featured p-1 flex flex-col md:flex-row gap-5 mb-5 items-stretch">
        <?php if ($recentNews != null): ?>
            <?php foreach ($recentNews as $key => $n): ?>
                <?php if ($key == 0): ?>
                    <div class="featured-main w-full aspect-video md:w-3/5 lg:w-8/12 shadow-custom rounded-xl overflow-hidden relative bg-center bg-cover hover:cursor-pointer flex items-end" data-id="<?= $n['id'] ;?>" data-img="<?= $n['hd']; ?>">


                        <div class="detail absolute bottom-0 left-0 w-full p-2  lg:p-10 text-white border-2">
                            <h2 class="text-lg  md:text-2xl   font-bold font-Montserrat">
                                <?= $n['title']; ?>
                            </h2>
                            <p ><?= $n['created_at']; ?></p>
                        </div>
            ...
            <?php endforeach; ?>
        <?php endif; ?>
        <div class="featured-nav w-full md:w-2/5 lg:w-4/12 bg-white p-5 rounded-3xl">
        ...
        <div class="berita-container w-full p-1 md:w-3/5 lg:w-8/12">
            <?php if ($recentNews != null): ?>
                <?php foreach ($recentNews as $key => $n): ?>
                    ...
                    ; ?>
                    <div class="berita-card hover:cursor-pointer w-full rounded-2xl overflow-hidden  bg-white/50 border flex mb-2"
                        ...
                        <div class="detail-berita p-2  w-9/12 ">
                            ...
                            <h2
                                class="judul-berita font-Montserrat font-bold text-gray-800 capitalize text-xs sm:text-sm break-words">
                                <?= $n['title']; ?>
                            </h2>
