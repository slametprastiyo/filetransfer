...
window.addEventListener("load", () => {
    let headerLeft =
        header.getBoundingClientRect().left - header.offsetWidth / 2;

    // header.style.width = header.offsetWidth + "px";
    // header.classList.add("mx-auto");
    header.style.left = headerLeft + "px";
});
window.addEventListener("resize", () => {
    let headerLeft = window.innerWidth / 2 - header.offsetWidth / 2;
...
export default function navbar() {
    ...
    const allDropdownParent = document.querySelectorAll(
